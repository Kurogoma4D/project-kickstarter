...
1. `.claude/`・`.agents/`・`.codex/` をプロジェクトルートにコピー（`.agents/skills` はシンボリックリンクなので、リンクを保ったままコピーする）
...
Codex のみで使う場合もファイル構成は同じで、スキルは `.agents/skills`、サブエージェントは `.codex/agents` から読み込まれる。
...
| `issue-worker` | `/issue-worker` | 単一 Issue を実装し1回レビューして PR を残す（マージしない） |
| `pentest` | `/pentest` | 静的解析と動的テストで脆弱性を検出し Issue を起票する |

| サブエージェント | 役割 |
|---|---|
| `code-reviewer` | PR のコード品質・バグ・設計をレビューする |
| `issue-implementer` | GitHub Issue を worktree で実装し PR を作成する |
